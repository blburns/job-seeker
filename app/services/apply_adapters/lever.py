...
class LeverAdapter:
    ...
    def can_handle(self, url: str) -> bool:
        lower = (url or '').lower()
        return 'lever.co' in lower or 'jobs.lever' in lower
    ...
    def _run_on_page(self, page: Any, context: ApplyContext, proof_path: str) -> ApplyResult:
        ...
        if not clicked:
            ...
            return ApplyResult(
                success=False,
                status='needs_manual',
                message='Could not find Lever Submit control; form may be pre-filled — finish manually.',
    ...
    def _fill_fields(cls, page: Any, fields: dict) -> None:
        for selector, value in [
            ...
            try:
                ...
                logger.debug('Could not fill Lever field %s', selector, exc_info=True)
    ...
    def _upload_resume(cls, page: Any, resume_path: Optional[str]) -> None:
        ...
        try:
            ...
            logger.debug('Could not upload Lever resume', exc_info=True)
    ...
    def _safe_screenshot(cls, page: Any, proof_path: str) -> None:
        try:
            ...
            logger.debug('Lever screenshot failed', exc_info=True)
